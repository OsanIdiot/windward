...
(async () => {
  ...
  try {
    ...
    const coasting = await saved();
    assert.equal(coasting.navigation.stopping, true);
    assert.ok(coasting.motion.speed > .9);
    ...
    assert.ok(E.N.distance(coasting.position, paused.position) > 5, 'Stopping retains forward drift');
