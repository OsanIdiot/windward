...
(async () => {
  ...
  try {
      ...
      window.AudioContext = class extends Native {
        constructor(...args) {
          ...
          this.createBufferSource = () => {
            ...
            source.start = (...args) => {
              if ([2.9, 2.8, 2.3].some(duration => Math.abs(source.buffer.duration - duration) < .001)) {
    ...
    const peak = await page.evaluate(() => turnProbe.creaks[0].peak);
    // Device-rate resampling may slightly exceed the encoded PCM peak.
    assert.ok(peak > .1 && peak < .6, `Recorded creak has audible, unclipped headroom: ${peak}`);
