...
These four assets are derivatives of user-supplied Pixabay recordings, not
...
All outputs: mono, 32,000 Hz, 16-bit PCM WAV; 85 Hz high-pass; 80 ms fade-in
...
`--inspect` reads the original files without writing outputs. The full recipe is
in `build-recorded-creaks.cjs`. Existing outputs are protected unless `--force`
